        ...
        <>
            {!props.seat.isFiller ? (
                <div id={`workstation${props.seat.seatId}`}>
                    <div
                        onClick={() => { (props.seat.status === WorkstationStatus.AVAILABLE || props.seat.status === WorkstationStatus.RESERVED || props.seat.editable) && openModal() }}
                        className={
                            "flex flex-col relative rounded border-2 h-20 w-28 cursor-pointer " +
                            colors[props.seat.status] + " " + themeClasses.flex
                        }
                    >
                        ...
                        {props.seat.type === WorkstationType.WINDOWS && <BorderAll className={themeClasses.icon} />}
                        {props.seat.type === WorkstationType.APPLE && <Apple className={themeClasses.icon} />}
                        {props.seat.type === WorkstationType.NEWDOCK && <LaptopWindows className={themeClasses.icon} />}
                        {props.seat.type === WorkstationType.OLDDOCK && <LaptopChromebook className={themeClasses.icon} />}
                        {props.seat.type === WorkstationType.WALL && <CancelPresentation className={themeClasses.icon} />}
                        {props.seat.type === WorkstationType.NONE && <NotInterested className={themeClasses.icon} />}
                        {props.seat.showTeamReservation && <label className="self-center">{props.seat.reservedByTeam}</label>}
                    </div>
                    <Modal
                        isOpen={modalIsOpen}
                    ...
                    >
                        <h1 className="text-xl self-center my-8">{"Seat " + props.seat.seatId}</h1>
                        ...
                        <label>{"Reserved by: " + (props.seat.reservedBy ? props.seat.reservedBy : "N/A")}</label>
                        <div className="flex w-full mt-8 justify-between flex-grow-2 items-end">
                            ...
                            <button disabled={props.seat.reservedBy} className={"rounded reserve " + themeClasses.button} onClick={reserve}>Reserve</button>
                        </div>
                    </Modal>
                </div>
            ) : (
...
const FloorPlanWorkstationRow = ({ props }) => {
    ...
    return (
        <div className="grid-cols-6 flex justify-evenly w-full">
            {data.map((datum, i) => (
                <FloorPlanWorkstation key={i} props={{ seat: datum, selectionUpdater: props.selectionUpdater, currentSelection: props.currentSelection }} />
            ))}
        </div>
    );
};
        ...
        <div className="flex w-full pb-4">
            ...
            <div className="flex flex-col w-full justify-center px-4 items-center bg-gray-300">
                {props.bayRows.map((row, i) => (
                    <FloorPlanWorkstationRow key={i} props={{ workstations: row, selectionUpdater: props.selectionUpdater, currentSelection: props.currentSelection }} />
                ))}
            </div>
        </div>
    );
    ...
    return (
        <div className="w-full">
            ...
            {props.sectorBays.map((bay, i) => (
                <div key={i}>
                    <FloorPlanBay props={{ bayLabel: bay.bayLabel, bayRows: bay.bayRows, selectionUpdater: props.selectionUpdater, currentSelection: props.currentSelection }} />
                </div>
            ))}
        </div>
    );
